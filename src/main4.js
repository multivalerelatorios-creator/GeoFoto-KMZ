...
const $=s=>document.querySelector(s), LOCAL='geofoto_offline_v2', CFG='geofoto_cfg_v1', IDENTITY='gf_identity', APP_VERSION='1.2.0'
...
function renderDashboard(){const hoje=new Date().toLocaleDateString('pt-BR'),n=points.filter(p=>new Date(p.time).toLocaleDateString('pt-BR')===hoje).length;$('#dashboard').innerHTML=`<div class="cards"><div class="card"><span class="muted">Total de pontos</span><div class="metric">${points.length}</div></div><div class="card"><span class="muted">Fotos hoje</span><div class="metric">${n}</div></div><div class="card"><span class="muted">Com endereço</span><div class="metric">${points.filter(p=>p.address).length}</div></div><div class="card"><span class="muted">KMZ central</span><div class="metric">OK</div></div></div><div class="grid2"><div class="card"><h3>Fluxo automático</h3><p class="muted">A câmera abre, o GPS é obtido e, ao tirar a foto, o ponto é salvo automaticamente no mapa e no KMZ.</p><button class="btn primary" id="quick">Abrir câmera</button></div><div class="card"><h3>Últimos registros</h3>${points.slice(-4).reverse().map(p=>`<p><b>${esc(p.name)}</b><br><span class="muted">${fmt(p.time)} · ${esc(p.city||'')}</span></p>`).join('')||'<p class="muted">Nenhum registro.</p>'}</div></div>`;$('#quick').onclick=()=>document.querySelector('[data-page="capture"]').click()}
function renderCapture(){raw='';photo='';geo=null;saving=false;savedPhotoKey='';const enabled=(cfg.enabledTemplates||Object.keys(TEMPLATES)).filter(id=>TEMPLATES[id]);const current=enabled.includes(cfg.defaultTemplate)?cfg.defaultTemplate:(enabled[0]||'essential');$('#capture').innerHTML=`<div class="grid2 mobile-main"><div class="card capture"><h3>Câmera de campo</h3><label class="field">Modelo da foto<select id="templateSelect">${enabled.map(id=>`<option value="${id}" ${id===current?'selected':''}>${TEMPLATES[id].name}</option>`).join('')}</select></label><div id="templateHint" class="muted small">${TEMPLATES[current]?.description||''}</div><label class="field"><b>Identificação do ponto *</b><input id="pointName" placeholder="Ex.: CEO 80551 / POSTE 023" autocomplete="off" autocapitalize="characters" spellcheck="false" required></label><div id="pointNameHint" class="muted small">Informe a identificação para liberar a câmera.</div><video id="camera" class="camera-large" autoplay playsinline muted></video><canvas id="canvas" class="hidden"></canvas><img id="preview" class="camera-large hidden"><div class="actions"><button class="btn primary" id="take">📷 Tirar foto</button><button class="btn secondary hidden" id="retake">↻ Tirar novamente</button></div><label class="field">Observação<textarea id="note" rows="3" placeholder="Descrição opcional"></textarea></label><div class="actions"><button class="btn secondary" id="gps">Atualizar GPS</button></div><div class="actions hidden" id="shareActions"><button class="btn primary" id="shareBtn">📤 Enviar</button><button class="btn secondary" id="downloadBtn">⬇ Salvar foto</button></div><div id="saveStatus" class="statusline">Aguardando foto...</div><div id="camInfo" class="muted">Abrindo câmera...</div><div id="gpsInfo" class="muted">Obtendo GPS...</div></div><div class="card"><h3>Modelo ativo</h3><div id="templatePreview" class="template-preview"><b>${TEMPLATES[current]?.name||'Essencial'}</b><p class="muted">${TEMPLATES[current]?.description||''}</p></div><p class="muted">A identidade do técnico é obrigatória e fica vinculada visualmente à foto registrada.</p></div></div>`;const pn=$('#pointName'),hint=$('#pointNameHint');pn.addEventListener('input',()=>{const start=pn.selectionStart,end=pn.selectionEnd;pn.value=pn.value.toLocaleUpperCase('pt-BR');try{pn.setSelectionRange(start,end)}catch{}hint.textContent=pn.value.trim()?'Identificação informada.':'Você também pode informar ao tocar em Tirar foto.'});$('#take').onclick=takePhoto;$('#retake').onclick=retake;$('#gps').onclick=getGps;$('#templateSelect').onchange=e=>{const t=TEMPLATES[e.target.value];$('#templateHint').textContent=t?.description||'';$('#templatePreview').innerHTML=`<b>${t?.name||''}</b><p class="muted">${t?.description||''}</p>`}}
async function startCameraAndGps(){await startCamera();getGps()}
async function startCamera(){const v=$('#camera');if(!v)return;try{stopCamera();stream=await navigator.mediaDevices.getUserMedia({video:{facingMode:{ideal:'environment'},width:{ideal:1920},height:{ideal:1080}},audio:false});v.srcObject=stream;await v.play();$('#camInfo').textContent='Câmera pronta.'}catch(e){$('#camInfo').textContent='Não foi possível abrir a câmera: '+e.message}}
async function takePhoto(){const pn=$('#pointName');let pointName=(pn?.value.trim()||'').toLocaleUpperCase('pt-BR');if(!pointName){pointName=(window.prompt('Identificação do ponto\nEx.: CEO 80551 / POSTE 023','')||'').trim().toLocaleUpperCase('pt-BR');if(!pointName){const hint=$('#pointNameHint');if(hint)hint.textContent='⚠ É necessário informar a identificação antes da foto.';return}if(pn)pn.value=pointName;const hint=$('#pointNameHint');if(hint)hint.textContent='Identificação informada.'}const v=$('#camera'),c=$('#canvas');if(!v||!v.videoWidth)return;const portrait=innerHeight>innerWidth;const outW=portrait?1080:1920,outH=portrait?1440:1080;c.width=outW;c.height=outH;const sx=v.videoWidth,sy=v.videoHeight,src= sx/sy, dst=outW/outH;let sw=sx,sh=sy,ox=0,oy=0;if(src>dst){sw=Math.round(sy*dst);ox=Math.round((sx-sw)/2)}else{sh=Math.round(sx/dst);oy=Math.round((sy-sh)/2)}c.getContext('2d').drawImage(v,ox,oy,sw,sh,0,0,outW,outH);raw=c.toDataURL('image/jpeg',.9);savedPhotoKey=raw.slice(0,80)+Date.now();stopCamera();$('#saveStatus').textContent='Foto tirada. Preparando dados e salvamento automático...';await annotate();captureUi();await maybeAutoSave()}
...
async function getGps(){const el=$('#gpsInfo');if(!el)return;if(!navigator.geolocation){el.textContent='GPS indisponível.';return}el.textContent='Obtendo GPS de alta precisão...';navigator.geolocation.getCurrentPosition(async p=>{geo={latitude:p.coords.latitude,longitude:p.coords.longitude,accuracy:p.coords.accuracy};const r=await reverse(geo.latitude,geo.longitude);geo.city=r.city;geo.address=r.address;el.innerHTML=`Latitude: <b>${geo.latitude.toFixed(6)}</b> · Longitude: <b>${geo.longitude.toFixed(6)}</b> · Precisão: <b>${Math.round(geo.accuracy)} m</b><br><b>${esc(geo.city)}</b> · ${esc(geo.address)}`;if(raw){await annotate();captureUi();await maybeAutoSave()}},e=>el.textContent='Falha no GPS: '+e.message,{enableHighAccuracy:true,timeout:20000,maximumAge:0})}
...
async function downloadCurrentPhoto(){const src=photo||raw;if(!src)return;const blob=dataToBlob(src),name=`geofoto-${Date.now()}.jpg`,st=$('#saveStatus');try{const f=new File([blob],name,{type:'image/jpeg'});if(navigator.canShare&&navigator.canShare({files:[f]})&&navigator.share){await navigator.share({title:'Salvar foto',files:[f]});if(st)st.textContent='✓ Foto pronta para salvar/compartilhar no aparelho.';return}saveBlob(blob,name);if(st)st.textContent='✓ Download da foto iniciado.'}catch(e){try{const u=URL.createObjectURL(blob);window.open(u,'_blank');setTimeout(()=>URL.revokeObjectURL(u),30000);if(st)st.textContent='Abra a imagem e use Salvar/Download no navegador.'}catch{if(st)st.textContent='Não foi possível salvar a foto neste navegador.'}}}
...
async function checkForUpdate(manual=false){
 try{
  ...
  const v=await r.json();if(v.version&&v.version!==APP_VERSION){showUpdatePrompt(v.version);return true}
  if(swRegistration){await swRegistration.update();if(swRegistration.waiting){showUpdatePrompt(v.version||'nova versão');return true}}
...
async function initUpdater(){
 ...
 try{
  ...
  if(swRegistration.waiting)showUpdatePrompt('nova versão');
  swRegistration.addEventListener('updatefound',()=>{const w=swRegistration.installing;if(!w)return;w.addEventListener('statechange',()=>{if(w.state==='installed'&&navigator.serviceWorker.controller)showUpdatePrompt('nova versão')})});
