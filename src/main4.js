...
function renderSettings(){const enabled=cfg.enabledTemplates||Object.keys(TEMPLATES);$('#settings').innerHTML=`<div class="grid2"><div class="card"><h3>Identidade do usuário</h3><label class="field">Nome exibido (obrigatório) *<input id="identityName" value="${attr(identity||'')}" placeholder="Ex.: João Silva / Equipe 01" required></label><label class="field">Cor principal<input id="primaryColor" type="color" value="${attr(cfg.primaryColor||'#0f766e')}"></label><label class="field">Logo / banner (opcional)<input id="bannerFile" type="file" accept="image/*"></label><img id="bannerPreview" class="banner-preview ${cfg.banner?'':'hidden'}" src="${attr(cfg.banner||'')}"><div id="brandStatus" class="muted small">Escolha uma imagem e toque em Salvar identidade.</div><div class="actions"><button class="btn primary" id="saveCfg">Salvar identidade</button><button class="btn secondary" id="clearCfg">Remover logo</button></div></div><div class="card"><h3>Modelos de foto</h3><label class="field">Modelo padrão<select id="defaultTemplate">${Object.entries(TEMPLATES).map(([id,t])=>`<option value="${id}" ${id===cfg.defaultTemplate?'selected':''}>${t.name}</option>`).join('')}</select></label><div class="template-list">${Object.entries(TEMPLATES).map(([id,t])=>`<label class="template-option"><input type="checkbox" data-template="${id}" ${enabled.includes(id)?'checked':''}><span><b>${t.name}</b><small>${t.description}</small></span></label>`).join('')}</div><div class="actions"><button class="btn primary" id="saveTemplates">Salvar modelos</button></div></div><div class="card"><h3>Aplicativo</h3><div class="version-box"><span>Versão instalada</span><b>v${APP_VERSION}</b></div><p class="muted">As atualizações são recebidas sem reinstalar o aplicativo e não apagam fotos ou registros.</p><div class="actions"><span class="update-state">✓ Atualizações automáticas ativas</span><button class="btn secondary" id="syncPending">Sincronizar pendentes</button><button class="btn secondary" id="refresh">Atualizar dados</button><button class="btn secondary" id="logout">Sair</button></div></div></div>`;$('#logout').onclick=()=>{sessionStorage.removeItem('gf_token');sessionStorage.removeItem('gf_role');sessionStorage.removeItem(IDENTITY);localStorage.removeItem('gf_token');localStorage.removeItem('gf_role');localStorage.removeItem(IDENTITY);token='';role='user';identity='';loginView()};$('#refresh').onclick=async()=>{await syncDown();renderAll()};const installBtn=$('#installApp');if(installBtn)installBtn.onclick=installApp;$('#syncPending').onclick=async()=>{if(!navigator.onLine){alert('Sem internet. Os registros continuam salvos no aparelho.');return}const n=(await pendingAll().catch(()=>[])).length;if(!n){alert('Não há registros pendentes.');return}const sent=await syncPendingQueue();alert(sent?'Sincronização concluída: '+sent+' registro'+(sent===1?'':'s')+' enviado'+(sent===1?'':'s')+'.':'Não foi possível sincronizar agora.')} ;const persist=async()=>{saveCfg();if(role==='admin'){try{await api('/config',{method:'POST',body:JSON.stringify(cfg)})}catch(e){alert('Não foi possível atualizar a configuração central: '+e.message)}}};
const brandInput=$('#bannerFile'),brandPreview=$('#bannerPreview'),brandStatus=$('#brandStatus');
brandInput.onchange=async()=>{const f=brandInput.files?.[0];if(!f)return;brandStatus.textContent='Preparando novo logo...';try{pendingBanner=await optimizeBrandImage(f);brandPreview.src=pendingBanner;brandPreview.classList.remove('hidden');brandStatus.textContent='Novo logo pronto. Toque em Salvar identidade para confirmar.'}catch(e){pendingBanner='';brandInput.value='';brandStatus.textContent=e.message;alert(e.message)}};
$('#saveCfg').onclick=async()=>{const id=$('#identityName').value.trim();if(id.length<2){alert('Informe o nome do técnico/usuário.');$('#identityName').focus();return}identity=id;sessionStorage.setItem(IDENTITY,identity);localStorage.setItem(IDENTITY,identity);cfg.primaryColor=$('#primaryColor').value;const f=brandInput.files?.[0];if(f&&!pendingBanner){try{pendingBanner=await optimizeBrandImage(f)}catch(e){alert(e.message);return}}if(pendingBanner)cfg.banner=pendingBanner;saveLocalBrand({banner:cfg.banner||'',primaryColor:cfg.primaryColor});await persist();pendingBanner='';renderSettings();alert(role==='admin'?'Identidade e logo atualizados. O novo logo também foi salvo na configuração central.':'Identidade e logo salvos neste aparelho.')};
$('#clearCfg').onclick=async()=>{cfg.banner='';pendingBanner='';saveLocalBrand({banner:'',primaryColor:cfg.primaryColor});await persist();renderSettings()};$('#saveTemplates').onclick=async()=>{const ids=[...document.querySelectorAll('[data-template]:checked')].map(x=>x.dataset.template);cfg.enabledTemplates=ids.length?ids:['essential'];cfg.defaultTemplate=cfg.enabledTemplates.includes($('#defaultTemplate').value)?$('#defaultTemplate').value:cfg.enabledTemplates[0];await persist();alert('Modelos atualizados.')}}
