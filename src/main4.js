...
function loginView(){
 ...
 <div class="login-card login-card-hub">
  <div class="brand login-brand"><div class="logo">⌖</div><div><h1>GEOFOTO KMZ</h1><div class="muted">Registro georreferenciado de campo</div></div></div>
...
function renderCapture(){
 ...
 const current=['essential','compact','location'].includes(cfg.defaultTemplate)?cfg.defaultTemplate:'essential';cfg.defaultTemplate=current;
 $('#capture').innerHTML=`<div class="field-flow"><div class="card capture"><div class="capture-title"><div><h3>Novo registro</h3><span class="muted small">${esc(identity)} · foto + GPS + localização</span></div><span class="capture-model">${TEMPLATES[current].name}</span></div><label class="field"><b>Identificação do ponto *</b><input id="pointName" placeholder="Ex.: POSTE 023 / CAIXA 08 / PONTO A" autocomplete="off" autocapitalize="characters" spellcheck="false" required></label><div id="pointNameHint" class="muted small">Informe a identificação do local.</div><video id="camera" class="camera-large" autoplay playsinline muted></video><canvas id="canvas" class="hidden"></canvas><img id="preview" class="camera-large hidden"><div class="actions"><button class="btn primary field-main-action" id="take">📷 Tirar foto</button><button class="btn secondary hidden" id="retake">↻ Tirar novamente</button></div><label class="field"><b>Observação</b><textarea id="note" rows="3" placeholder="Descrição opcional"></textarea></label><div class="actions"><button class="btn secondary" id="gps">Atualizar GPS</button></div><div class="actions hidden" id="shareActions"><button class="btn primary" id="shareBtn">📤 Enviar</button><button class="btn secondary" id="downloadBtn">⬇ Salvar foto</button></div><div id="saveStatus" class="statusline">Aguardando foto...</div><div id="camInfo" class="muted small">Abrindo câmera...</div><div id="gpsInfo" class="muted small">Obtendo GPS...</div></div></div>`;
 const pn=$('#pointName'),hint=$('#pointNameHint');pn.addEventListener('input',()=>{const start=pn.selectionStart,end=pn.selectionEnd;pn.value=pn.value.toLocaleUpperCase('pt-BR');try{pn.setSelectionRange(start,end)}catch{}hint.textContent=pn.value.trim()?'Identificação pronta.':'Informe a identificação do local.'});
 $('#take').onclick=takePhoto;$('#retake').onclick=retake;$('#gps').onclick=getGps
}
...
async function annotate(){
 if(!raw)return;
 const img=await loadImg(raw),c=document.createElement('canvas'),x=c.getContext('2d'),id=cfg.defaultTemplate||'essential',t=TEMPLATES[id]||TEMPLATES.essential;
...
function renderRecords(){
 ...
 const filtered=recordTechnicianFilter==='__unknown__'?points.filter(p=>!String(p.technician||'').trim()):recordTechnicianFilter?points.filter(p=>String(p.technician||'').trim()===recordTechnicianFilter):points;
 ...
 $('#records').innerHTML='<div class="records-toolbar"><label><span>Filtrar por técnico</span><select id="technicianFilter">'+options+'</select></label><div class="records-filter-count"><b>'+filtered.length+'</b><span>de '+points.length+' registros</span></div></div><div class="records-list">'+(filtered.slice().reverse().map(p=>'<div class="card record-card"><div class="record-head"><div><b>'+esc(p.name)+'</b><div class="muted small">'+fmt(p.time)+' · '+esc(p.city||'')+'</div><div class="record-technician"><span>Técnico:</span><b>'+esc(p.technician||'Registro antigo sem identidade')+'</b></div></div><span class="badge">'+Math.round(p.accuracy||0)+' m</span></div>'+((p.photo||p.photoUrl)?'<img class="record-photo" data-photo-id="'+p.id+'" alt="Foto de '+attr(p.name)+'">':'')+'<div class="record-note"><b>Descrição:</b><span>'+esc(p.note||'Sem descrição informada.')+'</span></div><div class="muted small">'+esc(p.address||'Sem endereço')+'</div><div class="muted small">'+Number(p.lat).toFixed(6)+', '+Number(p.lng).toFixed(6)+'</div><div class="actions"><button class="btn secondary" data-map="'+p.lat+','+p.lng+'">Abrir Maps</button>'+((p.photo||p.photoUrl)?'<button class="btn primary" data-share="'+p.id+'">Enviar</button>':'')+(role==='admin'?'<button class="btn secondary" data-delete="'+p.id+'">Excluir</button>':'')+'</div></div>').join('')||'<div class="card">Nenhum registro para este técnico.</div>')+'</div>';
...
function renderSettings(){
 const mainModels=['essential','compact','location'];if(!mainModels.includes(cfg.defaultTemplate))cfg.defaultTemplate='essential';
 $('#settings').innerHTML=`<div class="simple-settings"><div class="card"><h3>Perfil</h3><label class="field"><b>Seu nome *</b><input id="identityName" value="${attr(identity||'')}" placeholder="Nome ou identificação" required></label><p class="muted small">Usado para identificar quem fez o registro.</p><button class="btn primary" id="saveIdentity">Salvar nome</button></div><div class="card"><h3>Foto</h3><label class="field"><span>Modelo padrão</span><select id="defaultTemplate">${mainModels.map(id=>`<option value="${id}" ${id===cfg.defaultTemplate?'selected':''}>${TEMPLATES[id].name}</option>`).join('')}</select></label><p class="muted small">Esse modelo será usado automaticamente na câmera.</p><button class="btn primary" id="savePhotoModel">Salvar modelo</button></div><details class="card optional-settings"><summary>Personalização opcional</summary><div class="optional-body"><label class="field"><span>Cor principal</span><input id="primaryColor" type="color" value="${attr(cfg.primaryColor||'#0f766e')}"></label><label class="field"><span>Logo / banner</span><input id="bannerFile" type="file" accept="image/*"></label>${cfg.banner?`<img class="banner-preview" src="${cfg.banner}">`:''}<div class="actions"><button class="btn primary" id="saveBrand">Salvar</button><button class="btn secondary" id="clearCfg">Remover logo</button></div></div></details><div class="card"><h3>Aplicativo</h3><div class="version-box"><span>Versão</span><b>v${APP_VERSION}</b></div><p class="muted small">Atualiza sem reinstalar e sem apagar registros.</p><div class="actions"><button class="btn primary" id="checkUpdate">Verificar atualização</button><button class="btn secondary" id="refresh">Atualizar dados</button><button class="btn secondary" id="logout">Sair</button></div></div></div>`;
 const persist=async()=>{saveCfg();try{await api('/config',{method:'POST',body:JSON.stringify(cfg)})}catch(e){if(role==='admin')alert(e.message)}};
 $('#saveIdentity').onclick=()=>{const id=$('#identityName').value.trim();if(id.length<2){alert('Informe seu nome ou identificação.');return}identity=id;sessionStorage.setItem(IDENTITY,identity);alert('Nome salvo.')};
 $('#savePhotoModel').onclick=async()=>{cfg.defaultTemplate=$('#defaultTemplate').value;cfg.enabledTemplates=['essential','compact','location'];await persist();alert('Modelo salvo.')};
 $('#saveBrand').onclick=async()=>{cfg.primaryColor=$('#primaryColor').value;const f=$('#bannerFile').files[0];if(f)cfg.banner=await fileData(f);await persist();alert('Personalização salva.')};
 $('#clearCfg').onclick=async()=>{cfg.banner='';await persist();renderSettings()};
 $('#checkUpdate').onclick=()=>checkForUpdate(true);$('#refresh').onclick=async()=>{await syncDown();renderAll()};
