...
const $=s=>document.querySelector(s), LOCAL='geofoto_offline_v2', CFG='geofoto_cfg_v1', IDENTITY='gf_identity', ACCOUNT='gf_account', LOCAL_BRAND='gf_brand_local', APP_VERSION='1.4.9'
...
async function shareBlob(blob,name,data={}){const f=new File([blob],name,{type:'image/jpeg'}),text=shareMessage(data),url=googleMapsLink(data.lat,data.lng),payload={title:'GeoFoto KMZ',text,files:[f]};if(url)payload.url=url;if(navigator.canShare&&navigator.canShare({files:[f]})){try{return await navigator.share(payload)}catch(e){if(e?.name==='AbortError')return;try{return await navigator.share({title:'GeoFoto KMZ',text,files:[f]})}catch(e2){if(e2?.name==='AbortError')return}}}saveBlob(blob,name);if(url){try{await navigator.clipboard.writeText(text);alert('Foto salva. O link do Google Maps foi copiado para você enviar junto.')}catch{alert('Foto salva. Local: '+url)}}}
